...
namespace GPUInstancer
{
    public static class GPUInstancerUtility
    {
            ...
            int detailIndex, int quadCount, GPUInstancerTerrainSettings terrainSettings, GameObject replacementPrefab = null)
        {
            ...
            if (detailPrototype.treeType != GPUInstancerTreeType.None || GPUInstancerConstants.gpuiSettings.isLWRP || GPUInstancerConstants.gpuiSettings.isHDRP)
            ...
            {
                if (GPUInstancerConstants.gpuiSettings.isLWRP)
                {
        ...
        {
#if UNITY_EDITOR
            EditorApplication.update -= ImportFoliageLWRPShaderPopup;
            EditorApplication.update += ImportFoliageLWRPShaderPopup;
#endif
        }
        ...
        public static void ImportFoliageLWRPShaderPopup()
        {
            ...
            if (Shader.Find(GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP) == null)
            {
                if (System.IO.File.Exists(GPUInstancerConstants.GetDefaultPath() + "Extras/GPUI_Foliage_LWRP_Support.unitypackage"))
                {
                    if (EditorUtility.DisplayDialog("GPUI Foliage LWRP Support", "You are using the Detail Manager with texture Prototypes in LWRP.\n\nDo you wish to import the LWRP support for the GPUI foliage shader?\n\nThis operation can take some time depending on your system.", "YES", "NO"))
                    {
                        Debug.Log("GPUI is importing Foliage LWRP shader...");
                        AssetDatabase.importPackageCompleted += OnFoliageLWRPShaderImportCompleted;
                        AssetDatabase.ImportPackage(GPUInstancerConstants.GetDefaultPath() + "Extras/GPUI_Foliage_LWRP_Support.unitypackage", false);
                    }
                }
                else
                    Debug.LogError("GPUI can not find GPUI_Foliage_LWRP_Support.unitypackage");
            }
            EditorApplication.update -= ImportFoliageLWRPShaderPopup;
#endif
        }
        ...
        {
#if UNITY_EDITOR
            if (foliageShaderPackageName == "GPUI_Foliage_LWRP_Support" && Shader.Find(GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP) != null)
                GPUInstancerConstants.gpuiSettings.AddShaderVariantToCollection(GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP);
            AssetDatabase.importPackageCompleted -= OnFoliageLWRPShaderImportCompleted;
#endif
        }
            ...
            for (int i = 0; i < detailPrototypes.Count; i++)
            {
                ...
                if (detailPrototype.usePrototypeMesh)
                {
                    ...
                    if (detailPrototypes[i].prefabObject.GetComponentsInChildren<MeshRenderer>()
                        .Any(r => r.sharedMaterial.shader.name == GPUInstancerConstants.SHADER_GPUI_FOLIAGE || r.sharedMaterial.shader.name == GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP))
                    {
                        for (int lod = 0; lod < runtimeData.instanceLODs.Count; lod++)
                        {
                            for (int r = 0; r < runtimeData.instanceLODs[lod].renderers.Count; r++)
                ...
                else
                {
                    ...
                    instanceMaterial = new Material(Shader.Find(GPUInstancerConstants.gpuiSettings.isLWRP
                        ? GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP : GPUInstancerConstants.SHADER_GPUI_FOLIAGE));
        ...
        public static GPUInstancerPrefabPrototype GeneratePrefabPrototype(GameObject go, bool forceNew)
        {
            ...
            if (prototype == null)
            {
                ...
                if (prototype.treeType != GPUInstancerTreeType.None || GPUInstancerConstants.gpuiSettings.isLWRP || GPUInstancerConstants.gpuiSettings.isHDRP)
                            ...
                            else
                            {
                                if (!warnings.Contains(mats[i].shader.name))
                                {
                                    ...
                                    if (originalAssetPath.EndsWith(".shadergraph"))
                                        warnings += mats[i].shader.name + " is created with ShaderGraph, but GPUInstacer Setup Node is not added. Please add this node to your graph's position output as the final step.";
                                    else
