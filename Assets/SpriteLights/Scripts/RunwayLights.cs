...
public class RunwayLights : MonoBehaviour {
	...
	public void Start(){
		...
		int seed = Mathf.RoundToInt(System.DateTime.Now.Millisecond); 
...
    	}
	...
	public void CreateRunwayLights(){
		...
		float smallerLightSize = 0.7f; 
		float largerLightSize = 1f;
		float scale = 1f;
		...
		*/
		...
		for(int i = 0; i < runways.Length; i++){
			...
			SetupApproachLights(ref directionalLightData, allApproachLightData, runways[i], randomBrightness, brightness, largerLightSize * scale);
			SetupTDZLights(ref directionalLightData, allApproachLightData, runways[i], randomBrightness, brightness, smallerLightSize * scale);
			SetupStrobeLights(ref strobeLightData, allApproachLightData, runways[i], strobeTimeStep, largerLightSize * 3f * scale);
			SetupThresholdLights(ref directionalLightData, runways[i], false, 0.6f, largerLightSize * scale);
			SetupCenterlineLights(ref directionalLightData, runways[i], randomBrightness, brightness, smallerLightSize * scale);
			SetupEdgeLights(ref omnidirectionalLightData, runways[i], randomBrightness, brightness, largerLightSize * scale);
			SetupPapiLights(ref papiLightData, runways[i], largerLightSize * scale);			
		}
		...
		GameObject[] lightObjects = SpriteLights.CreateLights("Runway directional lights", directionalLightData.ToArray(), directionalLightsMat, indexFormat);
		MakeChild(parentObject, lightObjects);

		lightObjects = SpriteLights.CreateLights("Runway omnidirectional lights", omnidirectionalLightData.ToArray(), omnidirectionalRunwayLightsMat, indexFormat);
		MakeChild(parentObject, lightObjects);

		lightObjects = SpriteLights.CreateLights("Runway strobe lights", strobeLightData.ToArray(), strobeLightsMat, indexFormat);
		MakeChild(parentObject, lightObjects);

		lightObjects = SpriteLights.CreateLights("Runway PAPI lights", papiLightData.ToArray(), papiLightsMat, indexFormat);
		MakeChild(parentObject, lightObjects);
	...
	private void SetupStrobeLights(ref List<SpriteLights.LightData> lightData, ApproachLightData allApproachLightData, RunwayData runwayData, float strobeTimeStep, float size){
	...
	private void SetupPapiLights(ref List<SpriteLights.LightData> lightData, RunwayData runwayData, float size){
