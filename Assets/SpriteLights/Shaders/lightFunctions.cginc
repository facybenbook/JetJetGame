...
//Set the scale to prevent the light to be smaller than one pixel.
inline float ScaleUp(in float distance, in float globalScale, in float scale, in float angleGain, in float minPixelSize){

    //This is derived from the function PixelSizeAndDistanceToDiameter() from SpriteLights.cs
    //minimumDiameter is the minimum diameter the light must have to prevent it from disappearing because it is smaller than one pixel.
    float minimumDiameter = minPixelSize * distance * globalScale * scale;

    //Clamp the diameter so it is never smaller than the actual light size.
    float scaledDiameter = clamp(minimumDiameter, scale * angleGain, 10000.0f);

    return scaledDiameter;
}

//Move the light up a bit so it doesn't intersect the ground when it is scaled up.
