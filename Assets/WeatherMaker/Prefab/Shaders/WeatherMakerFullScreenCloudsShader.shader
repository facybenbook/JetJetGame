	...
	SubShader
	{
			...
			{ 
				...
				if (_CloudCoverVolumetric > 0.0)
				{
					...
					if (unity_OrthoParams.w == 0.0)
					{
						...
						uint iterations = SetupCloudRaymarch(weatherMakerCloudCameraPosition, rayDir, depth, depth, volumetricSphereInner, volumetricSphereOutter,
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				fixed4 pixelColor = WM_SAMPLE_FULL_SCREEN_TEXTURE(_MainTex2, i.uv.xy);
				pixelColor.rgb = ComputeAtmosphericScatteringFog(i.uv.xy, _WorldSpaceCameraPos, GetFullScreenRayDir(i.forwardLine), i.forwardLine, fixed4(pixelColor.rgb, 1.0), GetDepth01(i.uv.xy)).rgb;
				return pixelColor;
			}
		...
		Pass
		{
			...
			#pragma multi_compile_instancing
			#pragma multi_compile WEATHER_MAKER_SHADOWS_ONE_CASCADE WEATHER_MAKER_SHADOWS_SPLIT_SPHERES
			#pragma multi_compile __ UNITY_LWRP
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				return ComputeAtmosphericLightShafts(i.vertex.xy, _WorldSpaceCameraPos, GetFullScreenRayDir(i.forwardLine), rayLength);
			}
